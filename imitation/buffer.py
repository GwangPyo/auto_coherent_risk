...
class RolloutBuffer:
    ...
    def append(self, state, action, reward, done, logp_pi, next_state):
        data = (state, action, reward, done, logp_pi, next_state)
        if state.shape != self.state_shape:
            n_env = len(state)
            for i in range(n_env):

                self._append(state[i], action[i], reward[i], done[i], logp_pi[i], next_state[i])

        else:
            self._append(*data)

    def _append(self, state, action, reward, done, log_pi, next_state):
        self.states[self._p].copy_(torch.from_numpy(state))
        self.actions[self._p].copy_(torch.from_numpy(action))
        self.rewards[self._p] = float(reward)
        self.dones[self._p] = float(done)
